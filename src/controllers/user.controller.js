...
const getUser = catchAsync(async (req, res) => {
  let testUser = await userService.getUserById(req.params.userId)

  if(!testUser){
    throw new ApiError(httpStatus.NOT_FOUND,'User not found');
  } else if(testUser.email !== req.user.email){
    throw new ApiError(httpStatus.FORBIDDEN,'User is not authenticated to view this resource')
  }else{
    if(req.query.q){
      let testData = await userService.getUserAddressById(req.params.userId)
      res.send({address:testData.address})
    }else{
      res.send(testUser)
    }
  }
